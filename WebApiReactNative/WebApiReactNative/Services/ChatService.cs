...
using WebApiReactNative.Models.Chat;

namespace WebApiReactNative.Services
{
    public class ChatService(AppDbContext context, IIdentityService identityService, ChatMapper mapper) : IChatService
    {
        public async Task<long> CreateChatAsync(ChatCreateModel model) { var userId = await identityService.GetUserIdAsync(); var chat = mapper.ToChatEntity(model, userId); context.Chats.Add(chat); await context.SaveChangesAsync(); return chat.Id; }
        public Task<List<ChatTypeItemModel>> GetAllTypes() { return context.ChatTypes.AsNoTracking().Select(ct => mapper.ToChatTypeItemModel(ct)).ToListAsync(); }
        public async Task<ChatMessageModel> SendMessageAsync(SendMessageModel model)
        {
            ...
            var isMember = await context.ChatUsers.AnyAsync(x => x.ChatId == model.ChatId && x.UserId == userId);
            if (!isMember) 
                throw new UnauthorizedAccessException("User is not in chat");
            var message = new ChatMessageEntity {
                ChatId = model.ChatId, UserId = userId, Message = model.Message 
            }; 
        ...
        {   
            ...
            {  
                ...
                (u.FirstName != null && u.LastName != null && (u.FirstName + " " + u.LastName).ToLower().Contains(search)));        
            }        
